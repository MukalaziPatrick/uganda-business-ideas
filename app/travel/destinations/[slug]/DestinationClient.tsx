  ...
  return (
    <div className="min-h-screen bg-brand-cream">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-2 text-xs text-brand-green flex gap-1">
        ...
        <span>›</span>
        <span className="text-brand-forest font-semibold truncate">{destination.name}</span>
      </div>
                ...
                {stay.cover_photo_url ? (
                  <div className="relative h-36 w-full">
                    ...
                    {stay.status === "featured" && (
                      <span className="absolute top-2 left-2 bg-brand-gold text-brand-forest text-[10px] font-black px-2 py-0.5 rounded-full">⭐ FEATURED</span>
                    )}
                  </div>
                ) : (
                  <div className="h-24 bg-gradient-to-br from-brand-forest to-brand-green flex items-center justify-center text-3xl">🏨</div>
                )}
                <div className="p-3">
                  ...
                  <div className="flex gap-2 mt-2">
                    {stay.whatsapp && (
                      <div onClick={(e) => { e.preventDefault(); window.open(`https://wa.me/${stay.whatsapp.replace(/\D/g, "")}`, "_blank"); }}
                        className="flex-1 bg-brand-forest text-white text-center text-xs font-bold py-1.5 rounded-lg hover:bg-brand-green">💬 WhatsApp</div>
                    )}
                    {stay.booking_com_url ? (
                      <div onClick={(e) => { e.preventDefault(); window.open(stay.booking_com_url!, "_blank"); }}
                        className="flex-1 bg-brand-gold text-brand-forest text-center text-xs font-bold py-1.5 rounded-lg hover:bg-brand-gold/85">🌐 Booking.com</div>
                    ) : (
                      <div className="flex-1 bg-brand-forest text-white text-center text-xs font-bold py-1.5 rounded-lg">📋 Request</div>
                    )}
                  </div>
                </div>
              </Link>
